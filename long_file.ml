...
let implode l =
  let s = Bytes.create (length l) in
    let rec list_loop x = function
       [] -> ()
     | i::t -> Bytes.set s x i; list_loop (x + 1) t
    in
      list_loop 0 l;
      Bytes.to_string(s)
  ...
  | Short b ->
      let l = Bigarray.Array1.create Bigarray.int8_unsigned Bigarray.c_layout (Bytes.length b) in
        for x = 0 to Bytes.length b - 1 do
          bigarray_unsafe_set l x (int_of_char (Bytes.get b x))
        done;
        l
  | Long b -> b
...
let bytes_of_input i o l =
  ...
  let s = Bytes.create l in
    ...
    if l <= Sys.max_string_length then
      ...
      bytes_of_string (Bytes.to_string s)
  ...
  and last = o + 1 - 1 in
    if o > max || o < 0 || last < 0 || last > max then raise (Failure "getinit") else
      ...
      | Short s ->
          ...
          | None -> for x = o to o + l - 1 
                       do 
                         i.output_byte (int_of_char (Bytes.unsafe_get s x)) 
                       done
